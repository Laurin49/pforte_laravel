                    ...
                    <form action="{{ route('commands.update', $command->id) }}" method="POST">
                        ...
                        <div class="mb-2">
                            ...
                            <input
                                ...
                                type="text" name="name" value="{{ old('name', $command->name) }}">
                            @error('name')
                        ...
                        <div class="mt-4">
                            ...
                            <textarea name="beschreibung"
                                class="block w-full mt-1 border-gray-300 rounded-md shadow-sm placeholder:text-gray-400 placeholder:text-right focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                                rows="8" placeholder="400">{{ old('beschreibung', $command->beschreibung) }}</textarea>
                            @error('beschreibung')
                                <span class="text-sm text-red-600">
                                    {{ $message }}
                                </span>
                                ...
                                <label class="block text-sm font-medium text-gray-700" for="monat">
                                    ...
                                    <select class="block w-full mt-1" name="monat">
                                        @foreach (range(1, 12) as $month)
                                            <option value="{{ $month }}" {{ $month == $command->monat ? 'selected' : '' }}>
                                ...
                                <label class="block text-sm font-medium text-gray-700" for="jahr">
                                    ...
                                    <select class="block w-full mt-1" name="jahr">
                                        @for ($jahr = date('Y'); $jahr > date('Y') - 6; $jahr--)
                                            <option value="{{ $jahr }}" {{ $jahr == $command->jahr ? 'selected' : '' }}>
                                                {{ $jahr }}
                                            </option>
                                        @endfor
                                    </select>
