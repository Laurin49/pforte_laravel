            ...
            <div class="w-full px-16 py-20 mt-4 overflow-hidden bg-white rounded-lg lg:max-w-4xl">
                <div class="flex justify-between w-full mb-6">
                    <div class="flex items-center justify-start ">
                        <h1 class="font-serif text-3xl font-bold">Neues Thema</h1>
                    </div>
                ...
                <div class="w-full px-6 py-4 bg-white rounded shadow-md ring-1 ring-gray-900/10">
                    <form action="{{ route('subjects.index') }}" method="POST">
                        @csrf
                        <!-- Name -->
                        <div>
                        ...
                        <div class="mt-4"
                            ...
                            <input
                                class="block w-full mt-1 border-gray-300 rounded-md shadow-sm placeholder:text-gray-400 placeholder:text-right focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                                type="text" name="titel" value="{{old('titel')}}">
                            @error('titel')
                            <span class="text-sm text-red-600">
                                {{ $message }}
                            </span>
                        ...
                        <div class="mt-4"
                            ...
                            <input
                                class="block w-full mt-1 border-gray-300 rounded-md shadow-sm placeholder:text-gray-400 placeholder:text-right focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                                type="text" name="stichpunkte" value="{{old('stichpunkte')}}">
                            @error('stichpunkte')
                            <span class="text-sm text-red-600">
                                {{ $message }}
                            </span>
                        ...
                        <div class="mt-4">
                            ...
                            <textarea name="kosten"
                                class="block w-full mt-1 border-gray-300 rounded-md shadow-sm placeholder:text-gray-400 placeholder:text-right focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50"
                                rows="4" placeholder="400"> {{old('kosten')}}</textarea>
                            @error('kosten')
                            <span class="text-sm text-red-600">
                                {{ $message }}
                            </span>
